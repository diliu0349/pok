...
int add_soft_task(uint64_t period, uint64_t time_capacity, uint8_t pri, uint64_t deadline, uint64_t soft_ddl) {
  uint32_t tid;
  pok_thread_attr_t tattr;
  memset(&tattr, 0, sizeof(pok_thread_attr_t));
...
void user_thread() {
  while (1) {
    ...
    if (number == '0') {
      ...
      add_task(400, 200, 30, 300);
    } else if (number == '1') {
      printf("Create Chatbot Task\n");
      add_soft_task(600, 300, 40, 250, 200);
    } 
    else if (number == '2') {
